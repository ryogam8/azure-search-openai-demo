...
@app.route("/api/log-analytics", methods=["GET", "POST"])
def log_analytics():
    # セッションからアクセストークンを取得
    access_token = get_bearer_token()
    print(access_token)
    # アクセストークンを使用してAPIリクエストを送信するコードを記述
    ...
    query = (
        ...
        or "SigninLogs | project TimeGenerated, ResourceId, Identity | top 3 by TimeGenerated"
    )
