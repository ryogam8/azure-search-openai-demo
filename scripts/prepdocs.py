...
default_creds = (
    ...
    if args.searchkey == None or args.storagekey == None
    else None
)
print(args.searchkey)
search_creds = (
...
if not args.skipblobs:
    storage_creds = (
        default_creds
        if args.storagekey == None
        else AzureKeyCredential(args.storagekey)
    )
...
def upload_blobs(filepath):
    ...
    with open(file=filepath, mode="rb") as data:
        blob_client = blob_container.upload_blob(
            name=f"{basename}.txt", data=data, overwrite=True
        )
...
def remove_blobs(filename):
    ...
    if blob_container.exists():
        if filename == None:
            ...
            blobs = filter(
                lambda b: re.match(f"{prefix}-\d+\.txt", b),
...
def create_sections(filepath):
    basename = os.path.splitext(os.path.basename(filepath))[0]
    f = open(filepath, "r")
    data = f.read()
    f.close()
    yield {
        "id": base64.urlsafe_b64encode(basename.encode()),
        "content": data,
        "sourcepage": basename + ".txt",
    }
...
if args.removeall:
    ...
    for filepath in glob.glob(args.files):
        if args.verbose:
        ...
        if args.remove:
            ...
            try:
                ...
                sections = create_sections(filepath)
                index_sections(os.path.basename(filepath), sections)
